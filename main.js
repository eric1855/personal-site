...
// Physics (cannon-es)
const { world, step: stepPhysics, groundMaterial, chassisMaterial } = createPhysicsEngine()

// World (ground + trees — with cannon-es static bodies)
createWorldObjects(scene, world, groundMaterial)

// Buildings (with cannon-es static bodies)
const { locations } = createLocations(scene, world)

// Car (cannon-es rigid body)
const { carState, preStep: carPreStep, postStep: carPostStep } = createCar(scene, world, chassisMaterial)
...
function loop(now) {
  ...
  carPostStep()            // sync Three.js mesh from cannon body
