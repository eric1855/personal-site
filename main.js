...
// Collision world (AABB buildings + circle trees + world boundary)
const { colliders } = createColliderWorld()

// Car (pure kinematic — no physics engine)
const { carState, preStep: carPreStep, postStep: carPostStep, didCollide } = createCar(scene, colliders)
...
function loop(now) {
  ...
  if (didCollide()) updateCamera(carState, true)
  else              updateCamera(carState, false)
