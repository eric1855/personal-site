...
src/effects/
  atmosphere.js           atmospheric effects — floating ambient particles (300 Points), building beacon glow columns (pulsing CylinderGeometry), ground ring effects (pulsing RingGeometry); AdditiveBlending throughout; update(dt) called each frame
src/entities/
  car.js                  car mesh + hybrid kinematic/dynamic step + dust particles + collision dust burst + useLoadedModel()/useProceduralMesh() for GLB model swap; auto-loads Kenney sedan-sports GLB from /assets/models/car.glb with procedural fallback; enhanced procedural mesh: spoiler, bumpers, side mirrors, door lines, exhaust, roof bars, side windows, hub caps; undercarriage glow (additive, pulsing); headlight beam cones (additive); suspension bobbing (sin wave keyed to distance); speed trail particle pool (12 reusable additive planes)
  worldObjects.js         ground mesh + tree visual meshes (returned for Rapier body sync) + stone paths connecting spawn to buildings (with edge trim & central roundabout) + 15 ground color patches + 8 mounds + 8 lampposts (emissive warm glow + point lights) + 6 benches + 13 rocks (icosahedron detail=0, with random companion rocks) + 8 flower patches (tulip-style clusters) + 3 directional signposts + perimeter stone wall fence (±40, with gaps at path openings & corner pillars)
  locations.js            portfolio buildings (unique low-poly architectural styles per building) + useLoadedModel()/useProceduralMesh() per building for GLB model swap. About = cozy cabin (pitched roof, chimney, porch, potted plants, mailbox). Projects = workshop/factory (sawtooth roof, garage door, smokestacks, gears, crates, barrel). Experience = office tower (4-floor window grid, floor dividers, rooftop antenna+AC units, entrance canopy, briefcase). Contact = post office (arched entrance, columns, mail slot, clock on parapet, standing mailbox, bench, envelope).
...
src/ui/
  ui.js                   proximity HUD prompt + minimap (canvas, top-right) + speedometer (bottom-left) + splash/intro screen (full-screen overlay, click/key to dismiss)
  popup.js                location content popup with scale-up/down animation + frosted overlay
