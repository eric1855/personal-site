...
// Collision dust burst
const BURST_PARTICLES   = 12
const BURST_LIFETIME    = 25

// Car circle collider radius (approximation of BoxGeometry 1 x 1.9)
...
export function createCar(scene, colliders) {
  ...
  // Collision callback — set by main.js so car.js doesn't depend on UI
  ...
  function preStep(keys) {
    ...
    if (colliders) {
      const { hit, hitPoint } = resolveCollisions(state, colliders, CAR_RADIUS)
      if (hit && hitPoint) {
        _emitDustBurst(dust, hitPoint, state)
        if (_onCollision) _onCollision()
      }
    }
  }
  ...
  function onCollision(cb) { _onCollision = cb }

  return { carState: state, preStep, postStep, onCollision }
}
...
// Burst of dust particles at collision point
function _emitDustBurst(dust, hitPoint, state) {
  const { particles } = dust
  const available = MAX_PARTICLES - particles.length
  const count = Math.min(BURST_PARTICLES, available)
  for (let i = 0; i < count; i++) {
    const angle = Math.random() * Math.PI * 2
    const spd   = 0.03 + Math.random() * 0.05
    particles.push({
      position: new THREE.Vector3(
        hitPoint.x + (Math.random() - 0.5) * 0.4,
        0.2 + Math.random() * 0.3,
        hitPoint.z + (Math.random() - 0.5) * 0.4
      ),
      velocity: new THREE.Vector3(
        Math.cos(angle) * spd,
