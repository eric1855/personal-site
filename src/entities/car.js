...
function buildCarMesh() {
  ...
  // Taillights — store refs for collision flash
  return { group, flGroup, frGroup, tlL, tlR, tlMat }
}
...
const FLASH_EMISSIVE_HIT  = 2.0
const FLASH_EMISSIVE_IDLE = 0.6
const FLASH_DECAY         = 0.85

export function createCar(scene, colliderWorld) {
  const { group: carGroup, flGroup, frGroup, tlL, tlR, tlMat } = buildCarMesh()
  scene.add(carGroup)

  const dust  = buildDustSystem(scene)
  const state = createCarState()

  let flashIntensity = FLASH_EMISSIVE_IDLE

  function preStep(keys) {
    stepCar(state, keys)
    if (colliderWorld) resolveCollisions(state, colliderWorld)
  }
  ...
  function postStep() {
    ...
    // Collision visual feedback — flash taillights
    if (state.collided) {
      flashIntensity = FLASH_EMISSIVE_HIT
    } else {
      flashIntensity += (FLASH_EMISSIVE_IDLE - flashIntensity) * (1 - FLASH_DECAY)
    }
    tlMat.emissiveIntensity = flashIntensity
