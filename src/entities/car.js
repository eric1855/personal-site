...
export function createCar(scene, colliders) {
  ...
  // Track whether a collision happened this physics frame (for camera shake)
  let collisionThisFrame = false

  function preStep(keys) {
    stepCar(state, keys)
    // Resolve collisions after position update
    if (colliders && colliders.length > 0) {
      collisionThisFrame = resolveCollisions(state, colliders)
    }
  }
  ...
  function didCollide() {
    return collisionThisFrame
  }

  return { carState: state, preStep, postStep, didCollide }
}
