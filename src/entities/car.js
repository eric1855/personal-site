...
const EMIT_SPEED_THRESHOLD = 1.0  // cannon-es speeds are larger than kinematic speeds
...
export function createCar(scene, world, chassisMaterial) {
  const { group: carGroup, flGroup, frGroup } = buildCarMesh()
  scene.add(carGroup)

  const dust = buildDustSystem(scene)
  const { carState, preStep: physicsPreStep, postStep: physicsPostStep, chassisBody } =
    createCarPhysics(world, chassisMaterial)
  ...
  function postStep() {
    ...
    carGroup.position.set(carState.position.x, carState.position.y, carState.position.z)
